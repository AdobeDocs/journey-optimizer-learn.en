---
user-guide-title: Get started with Journey Optimizer Loyalty for marketers
user-guide-description: Learn how to onboard to Adobe Journey Optimizer Loyalty, configure a challenge, apply and display it, and analyze its performance.
role: User
level: Beginner
breadcrumb-title: loyalty
breadcrumb-url: /docs/journey-optimizer-learn/loyalty/overview
auto-video-transcripts: true
---

# Get started with Journey Optimizer Loyalty for marketers {#loyalty}

+ [Get started with Journey Optimizer Loyalty for marketers](./overview.md)
+ Introduction to Loyalty {#introduction-to-loyalty}
    + [Discover what Journey Optimizer Loyalty is](./introduction-to-loyalty/discover-journey-optimizer-loyalty.md)
    + {hide-from-toc} [Create a loyalty challenge](./create-a-loyalty-challenge.md)
+ Configure your challenge {#configure-your-challenge}
    + [Create and publish your first loyalty challenge](./configure-your-challenge/create-and-publish-your-first-loyalty-challenge.md)
    + [Create tasks for your loyalty challenge](./configure-your-challenge/create-tasks.md)
    + [Configure rewards for your loyalty challenge](./configure-your-challenge/configure-rewards-for-your-loyalty-challenge.md)
    + [Create challenges and get insights with the Loyalty AI agent](./configure-your-challenge/create-challenges-and-get-insights-with-the-loyalty-ai-agent.md)
+ Apply and display your challenge {#apply-and-display-your-challenge}
    + [Build a challenge content card with a code-based experience](./apply-and-display-your-challenge/build-a-challenge-content-card-with-a-code-based-experience.md)
+ Analyze and report {#analyze-and-report}
    + [Track performance with the challenge report](./analyze-and-report/track-performance-with-the-challenge-report.md)
